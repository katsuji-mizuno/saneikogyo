...
$(function() {
  // スムーススクロール(右ナビゲーション)
  // const paginations = document.querySelectorAll(".top_pagination a");
  ...
  var paginations = document.querySelectorAll(".top_pagination a");

  //var foods_list = document.querySelectorAll('.foods');

  var nodes_paginations = Array.prototype.slice.call(paginations, 0);

  nodes_paginations.forEach(function(pagination, index){
    //console.log(elem);
    // pagination.addEventListener("click", e => {
    //   e.preventDefault();
  ...
  // スムーススクロール(右ナビゲーション)
  // var scroll_tops = document.querySelectorAll(".top_scroll");
  // //var nodes_scroll_tops = Array.prototype.slice.call(scroll_tops, 0);

  // scroll_tops.forEach(scroll_top => {
  ...
  nodes_scroll_tops.forEach(function(scroll_top, index){
  //scroll_tops.forEach(scroll_top => {
    // scroll_top.addEventListener("click", e => {
    $(scroll_top).on('click', function (event) {
      event.preventDefault();
      const targetId2 = event.currentTarget.dataset.href;
      const target2 = document.querySelector(targetId2);
      target2.scrollIntoView({ behavior: "smooth" });
    });
  });
  ...
  var nodes_sections = Array.prototype.slice.call(sections, 0);
  ...
  nodes_sections.forEach(function(section){
  ...
  function doWhenIntersect(entries) {
    var nodes_entries = Array.prototype.slice.call(entries, 0);
    //entries.forEach(entry => {
    nodes_entries.forEach(function(entry, index){
  ...
  $('.fullPageScroll').on('scroll',function(){
    for (var i = secArr.length-1; i>=0; i--) {
      if ($('.fullPageScroll').scrollTop() >= secArr[i]){
        ...
        changeColor(cur_logo,cur_pg_color,cur_menu_color);
        //アイコン制御
        // if ($('.panel').eq(i).find('.link_icon')[0]) {
        //   $('.panel').eq(i).find('.link_icon').each(function(index, elem){
        //     var delay = 0;
        //     if ($(elem).data('delay')) {
        //       delay = 0 + Number($(elem).data('delay'));

        //     }
        //     console.log(delay);
        //     // $(elem).delay(300).addClass('view').delay(1000).queue(function(next){
